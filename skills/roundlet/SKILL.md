...
- Bind every Worker and Supervisor turn to explicit issue, pull-request, phase, review epoch/round, and full commit SHA context. Bind every canary result to the task-metadata-verified exact `model=<id>;reasoning_effort=<effort>` profile. Require Launcher results to match the exact expected task/model/effort in the owner-delivered activation or recovery prompt, and require Orchestrator/Worker migration canaries to match the candidate resolved role configuration. Bind each Supervisor turn and result to its attempt number and configured attempt profile. Require each role to reread the live sources defined by the role contract.
