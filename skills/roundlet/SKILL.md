...
6. Read-only select exactly one issue; provision and read back one unpublished local `codex/` branch plus isolated worktree; run the Orchestrator canary bound to that existing provisional substrate; then create the intended persistent Worker and run its worktree/index canary there. Make no source edit, commit, push, GitHub selection, or pull request until the complete same-phase aggregate is immutably stored and read back; only then record selection on GitHub and send implementation to that same Worker. Keep that Worker task through its lifecycle.
