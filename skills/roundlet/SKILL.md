...
- no Worker confidence, moving branch identity, file writes, GitHub, `gh`, web, or network; prior conclusions are never authority or proof, while bounded earlier finding/repair summaries may be supplied only as independent recheck targets in `CONVERGING` mode;
- strict `RESULT: PASS` or `RESULT: FINDINGS` output.

Before any Supervisor task-service mutation, call the pure `preflight_supervisor_creation`; it enforces the activation-bound `max_supervisor_cycles` budget and derives the immutable, round-aware convergence guidance without making the final draft-stage review unreachable. Pass the exact guidance and digest from that preflight to the creation request and Supervisor-only prompt; never reread config or reconstruct it for an active activation. `COMPLETE` performs broad independent falsification; after the configured count of completed reviews, `CONVERGING` focuses on earlier repairs and independently reproducible blockers without suppressing new P0/P1/P2 findings. Create through `create_supervisor_after_preflight` with its durable single-writer intent and stable idempotency key. If creation may have happened before the receipt was durably recorded, reconcile/read it back; never invoke the task service a second time. Bind its exact task ID, service creation timestamp, model/reasoning, project, parent/fork identity, permission profile, filesystem-write state, and connector/`gh`/web/network capabilities. Never synthesize or copy that receipt, accept an older/non-monotonic creation time, fork the Worker, reuse a previous Supervisor ID, or accept workspace-write/GitHub-capable metadata.
