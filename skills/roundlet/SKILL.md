...
6. Claim exactly one issue, record selection on GitHub, create one `codex/` branch and isolated worktree, then create the persistent Worker and require its exact-task worktree/index canary plus canonical aggregate read-back before implementation; keep that same Worker task through its lifecycle.
