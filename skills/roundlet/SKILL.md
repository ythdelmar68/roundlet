...
5. Select exactly one leaf. Bind any repository-owned external-validation contract, optional lifecycle observation sink, and standing authority before provisioning. Create one unpublished `codex/` branch and isolated worktree, then create and verify one persistent Worker. On native Windows, create the Worker at its distinct anchor before the descendant worktree; on other hosts, use the ordinary worktree-first topology.
