...
- After every App-managed repository task archive, observe the complete task/registration/path cleanup predicate for at most 30 seconds; archival alone is not cleanup completion. Interim observations are read-only and nonterminal. Persist exactly one terminal cleanup result with actual elapsed wait and observation count when removal or the allowed empty-tombstone predicate succeeds, or when the deadline expires. Never rewrite that result from a later observation.
...
- After every Supervisor archive, observe the complete cleanup predicate for at most 30 seconds, then append exactly one terminal cleanup snapshot/result to the run-local task-worktree cleanup ledger before another Supervisor can be created.
