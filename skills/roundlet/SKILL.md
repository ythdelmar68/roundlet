...
6. Read-only select exactly one issue, run the Orchestrator canary, and provision one unpublished local `codex/` branch, isolated worktree, and intended persistent Worker solely as the exact canary substrate. Make no source edit, commit, push, GitHub selection, or pull request until that Worker's worktree/index result and the canonical aggregate are immutably stored and read back; only then record selection on GitHub and send implementation. Keep that same Worker task through its lifecycle.
