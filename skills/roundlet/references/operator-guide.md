...
Aggregate every transition's required role results as one canonical `roundlet-filesystem-canary-set/v1` JSON manifest. It contains exactly `schema`, `run_id`, `transition`, and `results`; `transition` is one of the phase values in the role result contract. Each `results` entry contains exactly `phase`, `role`, `role_task`, `host_route_fingerprint`, `advisory_surface`, `worktree_surface`, `index_surface`, `cleanup`, and `result_sha256`; `result_sha256` hashes the exact stored UTF-8 `FILESYSTEM_CANARY_RESULT` bytes without BOM or trailing newline. Sort entries by the unsigned UTF-8 byte tuple `(phase, role, role_task, host_route_fingerprint)`, reject duplicate tuples, serialize with the contract manifest's RFC 8785 rules, and identify the set by `sha256:<lowercase-hex>` of those canonical bytes. A valid set includes every role/surface required for its transition, marks every required surface `PASS`, marks only inapplicable surfaces `NOT_APPLICABLE`, records `cleanup: VERIFIED` for every entry, and verifies every result hash and route identity. Missing, extra, duplicate, stale, failed, or overwritten entries invalidate the whole set.

Required sets are: activation = Launcher advisory, short-lived activation Worker worktree/index, and live Orchestrator advisory; issue claim = current live Orchestrator advisory plus the newly created persistent Worker worktree/index; recovery with an active leaf = recovery Launcher when used, recovered live Orchestrator, and the same retained Worker on their applicable surfaces; legacy bootstrap or active in-place migration = the same Orchestrator advisory plus same retained Worker worktree/index; between-issue adoption = same Orchestrator advisory plus a short-lived candidate-configured Worker worktree/index; benchmark = every role and all three surfaces named by the benchmark plan. Persist and bind only the aggregate digest where a single evidence reference is required; keep the manifest and its exact result bytes in local recovery state. External cleanup after a failed result is environment repair only and never changes that result or makes its set valid.
...
6. Send that Worker the issue-claim canary contract from `thread-prompts.md` for its exact worktree. Require worktree/index PASS, cleanup VERIFIED, restored initial identities, and a valid aggregate evidence set with the current Orchestrator advisory result. On failure, enter `FILESYSTEM_CAPABILITY_BLOCKED` with the selected resources retained and make no implementation, push, or pull-request transition.
7. Only after that exact-task canary and aggregate read-back pass, send the Worker the initial implementation contract from `thread-prompts.md`.
