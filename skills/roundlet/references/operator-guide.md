...
- every Worker replacement bound is a positive integer, the run bound is not smaller than the per-leaf bound, and terminal confirmation observations are positive;
...
- Archive and settle the old task/worktree only after preservation and effect reconciliation. Recheck owner scope, current repository/host policy, STOP/revocation, exact candidate, one-active-generation invariant, and both pinned replacement budgets before provisioning.
- Replacement consumes the per-leaf and per-run counters exactly once at the recorded creation intent. Failure or outcome-unknown creation does not refund a counter or permit another request. Exhaustion retains all work and enters `NEEDS_OWNER_INPUT`.
...
- A conclusively terminal physical Worker may be replaced automatically only through the logical-generation chain and pinned budgets. An active, inaccessible-but-not-terminal, denied, outcome-unknown, or unreconciled Worker is not replaceable.
