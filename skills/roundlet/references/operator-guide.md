...
- [Logical Worker generations](#logical-worker-generations)
...
- one logical Worker for that leaf and at most one active physical generation;
...
- asynchronous task creation exposes a stable operation/client identity and can be reconciled to one final task ID within `task_creation.reconciliation_seconds` without issuing a duplicate request;
...
- the owner-created root can resolve the explicitly invoked installed Roundlet skill to one canonical absolute root and read the exact required file set without an alternative-root scan or substitution;
...
- Worker continuation policy is exactly `until-active-leaf-terminal`, `max_active_generations` is exactly `1`, terminal confirmation observations are positive, and legacy numeric replacement caps are absent;
...
An external-action-free preflight defect stays in implementation and returns to the current generation of the same logical Worker or repository-owned toolbox correction. It is not `NEEDS_OWNER_INPUT`, does not consume a Supervisor attempt or review round, and does not publish `VALIDATION_READY` or repeated LIVE trace. Publish one readiness event only at `PREFLIGHT_READY`, and one result event only after `VERIFIED` (or one durable terminal blocked result after a genuinely armed invocation). Use a structured connector when available; a CLI fallback must use an exact body file, never shell-interpolated Markdown, and must read the event marker and required fields back semantically.

Repository standing authority and host command execution are separate. A failure before the selected command reaches GitHub, a provider, or a target does not prove repository authority or credentials are missing. Classify it under Execution blockers below: only a technical failure without a policy denial can use existing bounded, permitted recovery. An explicit policy denial cannot be retried through another role, tool, or route.
...
The owner-created root task is not created by Roundlet. Bind it once through `ROUNDLET_ROOT_ACTIVATION_BINDING` from `launcher.md`; never fabricate creator metadata for it. For every route probe, Worker generation, and Supervisor:

1. Before calling create, persist one `ROUNDLET_TASK_CREATION_INTENT` with a stable intent ID, creator/source task, requested role, exact configured model/effort, route, saved project, repository/common directory, existing starting ref, expected full SHA, run/leaf, and either Worker logical ID/generation or Supervisor formal tuple. The intent begins at `INTENT`.
2. Issue exactly one create request and persist its returned operation/client identity as `REQUESTED`; while setup remains incomplete record `PENDING`. A `clientThreadId`, setup operation, and final `threadId` are distinct identities and must never be substituted for each other.
3. Reconcile the same operation for at most `task_creation.reconciliation_seconds`. Only immutable creator read-back that maps the intent and operation to exactly one final role task may record `BOUND`. A timeout, transport break, or unknown response records `OUTCOME_UNKNOWN`; do not issue another create. If the host cannot recover the result by stable operation identity, enter `NEEDS_OWNER_INPUT` with reason `TASK_CREATION_CAPABILITY_GAP`.
4. After `BOUND`, send exactly one role task the `ROUNDLET_ROLE_METADATA_REPORT_REQUEST` from the role contract as its first prompt. That metadata-only turn performs no role, repository, GitHub, Git, heartbeat, or filesystem action.
5. Treat any `ROUNDLET_ROLE_METADATA_REPORT` or other returned rendering as advisory. It may be absent, one line, reordered, noncanonical, or omit a self-reported task ID.
6. Independently read immutable creator-side metadata before the first populated role prompt. Construct a proposed `CREATOR_TASK_BINDING_ATTESTATION` containing the role task ID, creator/source task, requested role, configured model/effort, route, requested saved project, actual project/workspace and canonical CWD, Git common directory and starting ref/SHA where applicable, available stable host/environment identity, and the stable creation intent/operation identities.
7. If the role report explicitly contradicts the proposed attestation, perform exactly one bounded creator-side immutable re-read. If authoritative values remain complete and match, preserve the attestation and treat the report as untrusted prose. Otherwise archive the task and fail closed before role work.
8. Require every authoritative field to match the creation request, then record exactly one creator attestation and copy it into the child role envelope/advisory state. Recovery and restart reuse it; neither path creates a second task, intent, attestation, dispatch, or availability event.

Role text never satisfies, alters, or invalidates binding by itself. A title, timestamp, worktree path, or role report cannot reconcile an outcome-unknown creation without stable operation-to-task identity. Missing, short, reordered, differently rendered, title-like, summary-like, or task-ID-free role output proceeds normally when creator read-back matches. Do not repeat task discovery on every turn. Later turns remain bound to the one verified attestation and carry current issue/phase/SHA plus Worker generation or Supervisor tuple.
...
5. Reuse the current Worker generation for every repair, main integration, final repair, and cleanup preflight while its context remains addressable. Never create a replacement for a reachable, running, delayed, or merely quiet Worker.
...
## Logical Worker generations

One selected leaf owns one stable logical Worker ID. Physical tasks are generations `1..n`; at most one generation may be `ACTIVE`. Persist for every generation its task, creation intent/operation, creator attestation, worktree, starting ref/SHA, current candidate lineage, formal review/final-repair tuple, dispatch cursor, and terminal cleanup result.

Use this recovery chain mechanically:

`ACTIVE(g) -> QUIESCING -> PRESERVED -> RECONCILED -> REPLACEMENT_READY -> PROVISIONING(g+1) -> ACTIVE(g+1)`

- Enter `QUIESCING` only after `worker_recovery.terminal_confirmation_observations` fresh, independent host observations prove the physical context/session terminal and unable to receive a turn. UI silence, elapsed time, a summary, or a failed wait alone is insufficient.
- Before `PRESERVED`, prohibit further dispatch and inventory commits, dirty/index/untracked content, exact refs/SHAs, validation/results, current objective/findings, final-repair state, and all pending task/Git/GitHub/external effects. Preserve unique bytes and commits under the declared local retention boundary or another exact recoverable ref. The Orchestrator constructs the checkpoint from live evidence; old Worker prose is optional corroboration.
- Before `RECONCILED`, read every pending effect back from its authoritative surface. Unknown execution remains pending and must not be retried. A late result from generation `g` is stale after `REPLACEMENT_READY` and cannot mutate state.
- Archive and settle the old task/worktree only after preservation and effect reconciliation. Recheck owner scope, current repository/host policy, STOP/revocation, exact candidate, one-active-generation invariant, the same nonterminal active leaf/PR lifecycle, and an incomplete authorized objective before provisioning.
- Replacement has no numeric per-leaf or per-run counter. Every generation and creation intent remains durably enumerated, but generation count never authorizes duplicate creation, policy bypass, or Supervisor-round extension. A failed or outcome-unknown creation is reconciled under its same stable intent and never blindly retried; an unrecoverable outcome remains `NEEDS_OWNER_INPUT`.
- Generation `g+1` must verify the preserved checkpoint and complete missing validation before continuing. It inherits the same logical objective, candidate history, accepted Supervisor results, review epoch/round/attempt, external/lifecycle sequences, and single final-repair allowance. It cannot manufacture PASS or reset a Supervisor attempt/round limit.
...
`current.md` records only bounded recovery facts: run/contract IDs, bundle, root activation binding, phase, issue and umbrella numbers/URLs, pull request, logical Worker/current generation/current Supervisor, every child role's creator binding, every creation intent/operation/final task ID and state, task route, requested saved-project identity, actual worktree/CWD, canonical Git common directory, starting ref/SHA, verified activation route-probe receipt/retained-ledger identity, base and candidate full SHAs, the completion-bound Worker continuation policy plus complete generation ledger, preserved checkpoint/retained-work identity, pending external effect, the run-local task-worktree cleanup-ledger identity, repository validation-toolchain summary/cache root and last public-safe lock/receipt status when applicable, selected external-validation route plus authoritative instruction/skill/target/action identities, declared readiness/result schema identities, opaque external-validation sequence, and historical evidence-time binding when applicable, optional lifecycle-sink contract/state plus plan/window/sequence/head/receipt identities, the separate formal review epoch/round/mode/attempt/profile and immutable ten-round maximum, last verified Supervisor-result event, last verified Worker-repair-handoff event, last durable transition UTC, last provider activity UTC, awaited condition, next wake/check, blocking scope, heartbeat interval, last full reconciliation, and bounded observation state. Never alias or copy values between Worker generation state, an external-validation or lifecycle-sink sequence, and the formal review tuple. Never store the advisory role report, credential, raw external payload, raw sink event, raw receipt, transcript, issue body, raw comment, diff, log, or reasoning.
...
For a new activation, the owner-created root binding names one canonical installed root and preflight binds that root, this exact seven-path set, and every per-file byte identity before repository access. Immediately before materialization and again before finalizing the contract path, require the same root, path set, and identity map. Each materialized bundle file must equal its preflight identity. This applies to exact verified blob identities for `git` and exact installed bytes for `installed-tree`; same-path drift or a mixed-generation bundle fails closed without a finalized contract or advisory state.
...
Every tick is a short durable checkpoint. Complete one bounded observation/read-back, make at most one external transition, persist the result, and end the turn. Do not keep a turn open merely to watch a role. Record task UI state separately from business outcome and provider activity; never infer a fixed context-expiry duration from silence. Each tick/status must expose the last durable transition UTC, last provider activity UTC, current role task and Worker generation, formal tuple, pending creation/effect, awaited condition, next wake/check, Worker continuation/generation state, Supervisor attempt/round state, blocker scope, retained work, and cleanup status.
...
`ACTIVATING`, `IDLE`, `ISSUE_PROVISIONING`, `ISSUE_SELECTED`, `IMPLEMENTING`, `WORKER_QUIESCING`, `WORKER_PRESERVED`, `WORKER_RECONCILING`, `WORKER_REPLACEMENT_READY`, `DRAFT_PR`, `REVIEWING`, `WAITING_CHECKS`, `MERGE_READY`, `MERGED`, `CLEANING`, `CLEANUP_BLOCKED`, `NEEDS_OWNER_INPUT`, `REPOSITORY_AUTHORITY_REQUIRED`, `OWNER_ABORT_DECISION_REQUIRED`, `PAUSED`, `STOP_AFTER_CURRENT`, and `STOPPED`.
...
2. Resolve and verify any repository-owned external-validation route, optional lifecycle-sink contract, standing authority, completion-bound Worker continuation policy, independent ten-round Supervisor limit, and the one saved local project matching the authoritative repository; then create one unpublished local `codex/` candidate ref from exact `origin/main` and one stable logical Worker ID.
3. Record the generation-1 creation intent and create it once asynchronously through the saved project's App-managed worktree route using that existing ref. Reconcile the stable operation/client identity to exactly one task ID; do not retry an unknown outcome.
4. After task readiness, independently verify detached `HEAD`, clean base SHA, candidate ref, App worktree registration/path/status, canonical Git common directory, logical Worker/generation/task/profile/route/requested-project/actual-CWD binding, non-reuse of every retained tombstone path, repository instructions, and absence of conflicting resources.
...
6. Only after successful read-back, publish the selection trace to the leaf issue, read it back there, enter `ISSUE_SELECTED`, and send the initial implementation prompt to the bound generation of that logical Worker.

The current generation may edit only its assigned worktree and logical Worker scope. It must reread root instructions, use conventional branch/commit rules, preserve unrelated user work, validate proportionally, and return a structured handoff. If it becomes conclusively terminal, do not reinterpret the failure as permission to create another task; follow Logical Worker generations first.
...
| Worker generation terminal/preservation/replacement event before a pull request exists | Issue |
...
| Worker generation terminal/preservation/replacement event after a pull request exists | PR Conversation |
...
```html
<!-- roundlet:event=<event-id>;run=<run-id>;worker-generation=<number-or-0>;epoch=<number>;round=<number-or-0>;candidate=<full-sha-or-none> -->
```
...
Record selection/ranking, Worker handoffs, bounded generation terminal/preservation/replacement outcomes, draft PR creation, verified external-validation readiness/result, invalid Supervisor availability attempts, valid Supervisor results, repairs, terminal review, owner/authority/abort decisions, merge, leaf closure, and cleanup. A Worker-recovery trace contains only logical/generation IDs, checkpoint digest, preserved candidate SHA, continuation-policy identity, one-active-generation status, and disposition; never private paths or raw task output. Keep provider-free executor construction, lifecycle-sink arm/append traffic, and preflight corrections local unless they change the candidate or become one durable blocker; publish at most the repository-allowed arm/seal identities as part of the existing validation readiness/result events. Summarize files, tests, findings/dispositions, SHAs, and risks. Never publish hidden reasoning, credentials, private artifacts, raw sink events, raw task transcripts, or unbounded logs.
...
Derive review mode mechanically from the immutable activation bundle. Require integer bounds with `1 <= review.complete_rounds <= review.max_rounds`; invalid bounds fail activation/recovery before review. Let `complete_rounds` equal pinned `review.complete_rounds` and `max_rounds` equal pinned `review.max_rounds`. A formal round outside `1..max_rounds` is not dispatchable. A round in `1..complete_rounds` projects to `COMPLETE`; a later round through `max_rounds` projects to `CONVERGING`. Treat every mode in advisory state, recovery evidence, a task request, or a populated prompt as a cached projection only, and require exact equality with this calculation before role work. Never choose, preserve, or advance mode independently.
...
1. Require the formal round to be in range, derive mode from the pinned config, and hold epoch, round, derived mode, and candidate SHA fixed.
2. Before task creation, semantically read back the candidate's canonical dispatch basis. For round 1, require the initial Worker handoff/draft-PR creation receipts and live PR head to identify the exact candidate. For every later round, require the immediately preceding accepted FINDINGS receipt, same-Worker repair handoff, non-force push and exact remote-head read-back, and canonical PR Conversation repair/candidate-movement receipt to agree on the preceding tuple, next round, and repaired candidate. If any item is missing, stale, duplicated, routed incorrectly, or contradictory, remain pending and reconcile only that item; create no Supervisor and consume no attempt or round.
3. Select the configured attempt profile at the exact one-based position.
4. Update the existing local candidate ref to the exact candidate SHA, record one Supervisor creation intent for the fixed tuple, then issue one asynchronous create through the same saved project's App-managed worktree route with that exact profile/ref. Reconcile the operation to exactly one final task ID before independently verifying its distinct detached worktree, canonical repository common directory, `HEAD`, non-tombstone path, and creator binding attestation. An outcome-unknown create does not advance to another attempt/profile or issue a duplicate. Do not interpret its advisory role report as binding evidence.
5. Record an independent read-only snapshot of that exact worktree: task/CWD/registration identity, detached `HEAD`, full candidate SHA, porcelain-v2 tracked/untracked/index status, and the exact candidate ref OID. Require clean status. Do not digest unrelated refs from the shared Git common directory.
6. Populate one shared envelope, one `SUPERVISOR_DISPATCH_ATTESTATION`, one `SUPERVISOR_REVIEW` block, and one required `SUPERVISOR_RESULT` schema instruction from the same live values. Immediately before sending, verify all start/end markers occur exactly once; every dispatch field is populated; the creator task binding is exact; the attempt profile is the configured position; the mode equals the config projection; repository, leaf, PR, base, candidate, ref, prior-result, and movement-receipt identities agree across repeated fields; and the required result marker/end marker and canonical PR Conversation route are present. A failed attestation sends no review work, consumes no attempt or round, publishes no result, archives the unused task, completes the mandatory task-worktree cleanup read-back/ledger entry, and returns to reconciliation without owner input unless a separate existing owner-input class is proven.
7. Send that attested read-only review prompt and require a structured result bound to the attempt/profile/epoch/round/derived-mode/SHA.
8. Before accepting any result, repeat the independent snapshot and require exact equality. Any HEAD, tracked/untracked/index, exact-candidate-ref, registration, common-directory, or CWD drift is `INVALID_CONTEXT`; the Supervisor's `read_only: true` field cannot override it. Unrelated shared refs are reconciled independently and never attributed to the Supervisor by this snapshot.
...
| Valid FINDINGS before the limit | Repaired by the current generation of the same logical Worker | Consumed; next round after repair | Reset to 1 | Publish/read back the Supervisor result, repair/push/read back the new candidate, publish/read back the repair handoff, then advance. |
| Valid FINDINGS at the limit | Final repair by the current generation of the same logical Worker | Consumed and terminal | No later attempt | Publish/read back the result and final-repair handoff, record `REVIEW_LIMIT_REACHED_WORKER_FINALIZED`, and dispatch no later Supervisor. |
...
Rounds through pinned `review.complete_rounds` are COMPLETE when reached; any valid PASS ends review. Later rounds through pinned `review.max_rounds` are CONVERGING and focus on prior findings/delta while allowing new blocking regressions or missing evidence.

Before round 10, valid findings go to the current generation of the same logical Worker for repair. The Orchestrator independently verifies the repair handoff/diff/tests, pushes the exact new candidate without force, reads back the remote head, appends and reads back the handoff trace in the PR Conversation, updates the pull request state, and only then advances to the next review round. Round-10 findings go once to `WORKER_FINAL_REPAIR`; a permitted generation replacement may complete that same final repair but cannot add another repair or round. The Orchestrator performs the same verify/push/read-back/trace sequence for that final candidate, but does not create round 11 or claim PASS. Record `REVIEW_LIMIT_REACHED_WORKER_FINALIZED` in the PR Conversation, then apply normal merge gates. If all configured attempts for a round fail, enter `NEEDS_OWNER_INPUT` and record/read back that lifecycle block on the leaf issue.

Worker generation and formal Supervisor accounting are disjoint state machines. A generation replacement before, during, or after a FINDINGS repair preserves the exact epoch, accepted-round count, current/next round, attempt position, candidate basis, and final-repair allowance. The absence of a numeric Worker generation cap never changes the ten-round Supervisor maximum and never permits a Supervisor dispatch at round 11.

## Owner input

### Execution blockers

Before escalating a denied dispatch or operation, reconcile its exact action, role, target, authoritative policy source, and visible tool decision once with read-only checks. Store a bounded local receipt: run/task/action identity, dispatch versus execution stage, policy commit/blob and owner-source reference, tool receipt ID, execution status (not executed, executed, or unknown), reported risk and authorization assessments, decision, and a concise reason. Mark unavailable fields `unavailable`; never infer a missing assessment or inspect hidden reasoning. A task summary that omits tool output does not prove success or absence of denial. Read the exact visible result when available; otherwise retain the uncertainty.

This classification precedes availability fallback and recovery for every role. An explicit policy denial of Supervisor creation/dispatch is not an invalid review attempt and cannot advance to another profile. Metadata-only creation carries no implementation delegation; if creation itself is denied, record that stage and do not claim the later populated authority envelope was presented. Keep metadata-only first turns unchanged.

Persist the bounded authority commit/blob and verified owner-source references with advisory state; do not store raw owner context. On recovery, re-resolve authoritative policy and reconcile those sources before any populated redispatch. Missing provenance blocks the affected action rather than borrowing a role's assertion; recovery never changes the pinned skill contract.

- **Missing authority:** name the exact absent/false switch or missing scope and use the existing repository-authority or owner-input boundary. Ask for the missing decision, not a blanket grant.
- **Host policy denial:** stop the denied action. If authoritative scope is adequate, record `HOST_POLICY_CONFLICT` under `NEEDS_OWNER_INPUT`; explain that repeating the same owner grant cannot resolve the observed denial. Preserve work and request only a supported policy/platform remedy or a genuinely different owner decision. Do not erase valid standing authority, repeatedly resend the rejected action, use Worker replacement, or change roles/tools/routes to evade the decision. A source-policy proposal is inert until reviewed, merged, and validly acknowledged; it is not a release receipt. Resume only after a supported remedy and fresh reconciliation establish that the action is permitted.
- **Technical execution failure without a policy denial:** use the existing narrow sandbox-approval/connectivity recovery when supported. Do not call it missing repository authority. Unknown execution or partial mutation requires read-back before any retry; exhausted recovery or an actual approval denial uses the existing blocker boundary.

When a visible host decision itself reports adequate authorization but denies the operation, preserve both facts. Neither a high authorization assessment nor a target policy switch overrides the host decision. Publish only the curated public-safe blocker summary if independently permitted; keep private task/path details and raw decision context local. No refused dispatch consumes a Supervisor review round or proves that Worker implementation occurred.
...
If `origin/main` advanced, reread mergeability and rules. Merge directly only when GitHub still accepts it and no rule requires an update. Otherwise send the current generation of the same logical Worker a main-integration turn that merges `origin/main` into its exact candidate checkout without rebase/force. The Orchestrator independently verifies the integration handoff/diff/tests, updates and pushes the exact candidate ref without force, reads back the remote head, appends and reads back the handoff trace in the PR Conversation, and then starts a new COMPLETE epoch.
...
2. If the current Worker generation is reachable, send it the read-only cleanup preflight. If it is conclusively terminal or already settled, the Orchestrator performs the same read-only Git/worktree/ref/unique-work checks independently from refreshed authoritative evidence; cleanup must not depend on an exhausted context. No path removes or edits the Worker's source during preflight.
3. Independently verify the preflight/checkpoint. Archive the current Worker generation only after all unique work is preserved and every pending effect is reconciled.
4. After its archive acknowledgement, run the bounded App-managed cleanup-settlement procedure and append its one terminal `ROUNDLET_TASK_WORKTREE_CLEANUP_RESULT` to the same run-local ledger. Retired generations must already have terminal entries from replacement.
5. Consume the complete run-local task-worktree cleanup ledger, then inventory every Orchestrator-created auxiliary worktree and state root. Require one terminal cleanup result for every physical Worker generation, Supervisor, and route probe task, plus a reconciled creation record for every requested operation. Classify every remaining resource as source-only or evidence-bearing from the exact repository-owned contract and live contents. For every unique evidence-bearing artifact, copy exact closed bytes into the repository-declared local retention boundary, record source identity, destination identity, size, and digest, and read every retained byte/size/digest back. Missing task entries, outcome-unknown creation/effects, ambiguous/open/changing resources, partially copied artifacts, or unverifiable evidence enters `CLEANUP_BLOCKED`.
...
9. Delete local/remote issue branches only when authorized, exact fresh refs and expected SHAs match, ancestry is proven, all generations are inactive/settled, and unique work is merged or explicitly abandoned. Record separate local-branch, remote-branch, and per-generation worktree results.
...
12. Append the cleanup trace to the leaf issue, including the bounded retention-manifest identity and complete resource inventory, read it back there, and clear issue pointers. If continuing, retain lease/contract and set `IDLE`; if stopping, append/read back `STOPPED` on the issue, remove/read back the heartbeat, remove advisory state and contract bundle after final reconciliation, persist the final stop receipt outside the removed run pointers, and only then archive the root Orchestrator.
...
If the root Orchestrator or heartbeat is inaccessible, use [Explicit recovery](launcher.md#explicit-recovery) from a new owner-created root task; do not create a recovery Launcher.
...
- An inaccessible root Orchestrator/heartbeat requires a new owner-created root task, a fresh direct recovery instruction, and the explicit recovery protocol. A stale-looking file is insufficient and provenance is never auto-transferred.
- A conclusively terminal physical Worker may be replaced automatically only through the logical-generation chain while the same active leaf/PR remains nonterminal and its authorized objective remains incomplete. There is no numeric generation cap. An active, inaccessible-but-not-terminal, denied, outcome-unknown, or unreconciled Worker is not replaceable, and replacement never alters the ten-round Supervisor limit.
- A failed Supervisor is disposable under the bounded attempt rule, except that an explicit host policy denial follows Execution blockers and cannot trigger another profile dispatch.
...
- Reconstruct each task creation intent/operation/final task identity and every physical Worker generation independently. Unknown creation or external effect stays blocked without duplicate retry.
