...
3. After all Git identity reads, restore the same captured bytes once more, then perform only raw filesystem read-back: require the final index length/SHA-256 to equal the preimage and require `index.lock` absent. Retain no backup or helper artifact. The subsequent direct-`apply_patch` file cleanup and, when applicable, two-turn empty-parent finalization remain separate.

Use the following PowerShell body verbatim for that one approved operation. This is a native-Windows Worker template, not a cross-platform command. Before populating or parsing it and before creating either canary artifact, derive the exact absolute worktree/index canary files and their immediate parents; prove all four paths absent with literal-path filesystem reads, prove both parents are distinct ordinary descendants inside but unequal to the linked-worktree root if later created, and record one immutable initial-absence digest binding all paths and results. Missing parent-absence evidence cannot be reconstructed after mutation and makes the empty-parent exception unavailable. Then populate only the eleven assignment values at the top, require every path value to contain no single quote or newline and both relative canary paths to contain no whitespace, and parse the fully populated body without executing it. Do not rewrite its functions, control flow, Git calls, restoration order, or output. Its strict Git wrapper temporarily permits native stderr capture because Windows Git may emit a warning while returning exit 0, then restores stop-on-error behavior and judges the subprocess only by the captured exit code; a real non-zero still throws. A path/preflight or parse failure occurs before canary mutation and is `FILESYSTEM_CAPABILITY_UNAVAILABLE`; never consume the approval or create an artifact after it. After the direct-`apply_patch` artifacts and exact content hashes are verified, submit that same parsed body as the single approved shell operation:
...
if ($IndexCanaryPath -match '\s' -or $WorktreeCanaryPath -match '\s') { throw 'canary path contains whitespace' }
