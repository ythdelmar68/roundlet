...
Roundlet supports **Python 3.12, 3.13, and 3.14** on **macOS** and **Windows 11 workstations** only. Windows Server and unproven future Windows builds are rejected; do not claim Linux, earlier Windows releases, another Python minor, or another Codex surface as supported until it has equivalent reviewed implementation and exact-candidate evidence. Before activation, run `<PYTHON> <installed-roundlet>/scripts/orchestration_state.py runtime-contract`; activation records and rechecks that host receipt before every guarded state/Git operation, rejecting missing, stale, or mismatched evidence before any Roundlet resource is created.
