...
The Launcher is a short-lived Codex task. It preflights one target repository, creates the configured long-lived Orchestrator task, attaches exactly one heartbeat to that task, transfers the heartbeat identity, and then archives itself. Before its metadata-only handshake, create it directly against the exact authoritative checkout as its canonical current directory and normal writable local-project workspace. Never use a projectless task, unrelated project, read-only route, or removable linked worktree for a Launcher. Independently read back that task/CWD/workspace binding, then send only the immutable task-metadata handshake from `thread-prompts.md`; after external creator-side read-back passes, send the populated activation prompt below as the second turn.
...
Read the complete Roundlet SKILL.md and every reference it requires before acting. Require the preceding immutable task-metadata handshake from `thread-prompts.md` to have exhausted deferred discovery and returned this exact Launcher task/model/effort, with mutually equal thread/session fields used for `role_task` and the prior `turn_id` recorded only as `metadata_turn`. Require the external creator's independent immutable task/turn-record read-back to equal those values and to prove that this task's canonical current directory is the exact authoritative checkout in its normal writable local-project workspace; a projectless task, unrelated project, read-only route, removable linked worktree, missing workspace read-back, or scoped approval in place of that topology fails before reservation or mutation. Missing eager exposure, self-reported profile text, task/turn substitution, or the child response alone is insufficient. Reread the immutable metadata route on this exact populated turn, require its task/profile/workspace to equal the owner-bound expected values above, bind the new turn separately, and bind the verified profile plus actual task ID into the Launcher canary result. Do not implement an issue in this Launcher task.
...
2. Verify that this Codex host can create, address, wait for, archive, and resume tasks; require this Launcher's creator-read-back canonical CWD to equal the authoritative checkout in its normal writable local-project workspace; exhaust deferred tool discovery and complete the two-stage immutable task-metadata handshake plus creator-side immutable task/turn-record read-back; on native Windows create a host-owned Worker task anchor with a separately removable writable descendant linked-worktree path; externally inspect task liveness, exact-current-directory holders, Git worktree registration, and physical candidate paths after archive; create, inspect, update, pause/resume, and stop one recurring heartbeat at every configured interval; select every configured model and reasoning effort in every Supervisor attempt profile exactly; and access Git, the target checkout, and GitHub with the required read/write capabilities. Do not substitute an unsupported model, effort, attempt profile, interval, metadata route, Launcher workspace, topology, or cleanup inspection.
...
8. Reserve one unguessable stable run ID before creating the contract bundle or any run resource, and use that exact ID in every activation canary result and later lease/current record; reservation alone grants no ownership. Run activation filesystem canaries before creating the contract bundle or run. In this correctly bound Launcher task, choose a separate unguessable artifact nonce and exact absent path below ignored `.roundlet/`; create, change, and delete the bounded artifact through direct `apply_patch` in the normal writable authoritative-checkout sandbox, read back each exact identity/content, remove any canary-created empty parent through the ordinary exact nonrecursive, non-force route, and prove path absence plus surrounding-state restoration. An out-of-root restriction proves invalid Launcher topology and cannot consume or invoke the configured retry as a substitute. A genuine initial restriction on the correctly bound route retains the configured typed retry contract. Do not apply the native-Windows Worker external empty-parent exception to the Launcher. On native Windows only, create the short-lived canary Worker task using the configured Worker model/effort, send only the metadata-handshake prompt, independently bind its exact host-owned task-anchor CWD, require a distinct writable descendant path for the Roundlet-owned worktree, and only then create the temporary isolated linked worktree there from authoritative `origin/main`; never make the task CWD equal to or inside that removable worktree. On WSL, Linux, macOS, and every other runtime, preserve the existing worktree-before-Worker topology: first create the temporary isolated linked worktree through the platform-appropriate route, then create the configured Worker task and send only its metadata-handshake prompt, with no task-anchor requirement. In both branches, independently read back the created task's immutable task/turn record and send the populated canary prompt only after exact task/profile and applicable topology equality. That task must reread immutable metadata on the canary turn; capture the complete raw-byte preimage of the resolved index path before the first operation that may lock or write it, then capture the remaining exact initial HEAD/status/index identities; create and change one unique bounded worktree artifact; read back identity/content; stage only one separate unique unignored canary path; verify its exact index entry/blob/content; unstage only that path; remove both artifacts; restore the exact index preimage through the bounded Git-index route without retaining a backup artifact; and prove final raw index length/SHA-256 after all remaining identity reads plus HEAD/status/tree/entries match the initial values. A semantic index match never substitutes for raw-byte equality. On native Windows only, if direct `apply_patch` file deletion leaves the exact qualifying canary-created empty parent, require `WINDOWS_CANARY_EMPTY_PARENT_READY`; independently verify its mutation-turn metadata and exact initially absent descendant/non-reparse/empty/restored-state evidence; remove only that parent with a nonrecursive, non-force exact-path directory operation; read back absence; independently reread the same stable Worker task/profile; then send one fresh metadata-gated read-only finalization turn. Accept only the canonical result from that finalization turn after creator-side exact-turn read-back and the Worker's repeated absence/index/worktree/no-transition verification. Never apply this exception to the Launcher, WSL, or a non-Windows runtime, a canary file, source, ancestor, sibling, worktree root, task anchor, `.roundlet` state, repository root, or user path. After artifact/index cleanup proof, keep the clean canary Worker and temporary linked worktree available and unchanged until the complete activation evidence set is built and read back; only then archive the Worker and apply the operator guide's bounded external task/worktree resource-cleanup contract. Use no user path, existing file, unique work, issue branch, commit, or GitHub mutation.
...
3. Record the same activation and active contract ID and bundle path in both advisory files. Create exactly one long-lived Orchestrator task using role `orchestrator` model and reasoning effort from the pinned bundle. Send only the metadata-handshake prompt first; independently read back its immutable task/turn record; and only after exact ID/profile equality send the populated bootstrap turn. Give it the exact target, checkout, run ID, owner allowlist, resolved authority, pinned configuration, contract ID and bundle path, lease/current paths, the verified exact Launcher and activation-Worker canary results plus their hashes as provisional inputs, and the Orchestrator contract from that bundle. Require this Orchestrator task to reread immutable metadata on that populated turn, repeat the unique advisory-state create/edit/read-back/cleanup canary on a new exact path, aggregate all three required results into the canonical activation evidence set, and read back its manifest/digest before readiness. When Launcher preflight relied on `gh`, require the Orchestrator to repeat the representative read-only request in its own task under the same automatic escalation and bounded recovery rules before it answers exactly:
   ACTIVATION_READY run=<run-id> target=<owner/repository> state=IDLE canary_set=<sha256>
   without selecting an issue yet.
4. Wait for that exact response and independently read back the bound aggregate. Capture the short-lived Worker's task/worktree cleanup baseline, archive it, and apply bounded external read-back. Require the task non-active, exact Git registration absent, no unique work, no live process with that exact canonical Roundlet-owned worktree as current directory, and the physical worktree path absent. On native Windows, separately record the distinct host-owned task anchor and any surviving exact-anchor CWD holder; that holder is not a worktree holder and does not invalidate cleanup after the separate worktree is absent. Never delete a nonempty anchor, kill Codex/Node, or force removal. This required post-acceptance resource cleanup does not alter the immutable activation result bytes or completed-transition set. If creation, preflight, acknowledgement, aggregate read-back, process/task/Git/path inspection, or resource cleanup is incomplete or ambiguous, stop and preserve evidence; do not attach a heartbeat.
...
Create the recovery Launcher directly against the exact authoritative checkout as its canonical current directory and normal writable local-project workspace. Reject projectless, unrelated-project, read-only, or removable-linked-worktree topology. Send only the immutable task-metadata handshake, independently verify its immutable task/turn record plus exact task/CWD/workspace binding, then copy and paste this entire prompt into that same task as the populated second turn:
...
Require the preceding immutable task-metadata handshake to have exhausted deferred discovery and returned this exact recovery Launcher task/model/effort. Require the external creator's independent immutable task/turn-record read-back to equal the requested values and `role_task`, and require its canonical CWD to equal the authoritative checkout in the normal writable local-project workspace; missing eager exposure, self-reported profile text, projectless/unrelated/read-only/removable topology, or the child response alone is insufficient. Reread metadata on this populated turn and bind the verified profile, workspace, and actual task ID into the canary result. Read only `.roundlet/lease.json` and the minimal contract commit records needed to resolve the effective active bundle, verify that bundle completely, then read its `SKILL.md` and every required reference before acting. Treat installed skill/configuration files only as a separately fingerprinted candidate. This prompt is explicit owner authorization to investigate and propose recovery; it is not authorization to discard, overwrite, merge, close, delete, or otherwise destroy old work.
...
7. Create exactly one replacement Orchestrator using the active pinned bundle's configured model and effort. Complete its metadata-only handshake and independent creator-side task-registry read-back before sending the populated recovery turn. Give it the reconstructed state, evidence, task identities, branch/worktree, candidate SHA, review epoch/round, current Supervisor attempt/profile when applicable, and the verified recovery Launcher plus applicable retained-Worker result bytes/hashes. Require it to reread immutable metadata, run its unique `RECOVERY` advisory canary, aggregate every applicable result into the canonical recovery evidence set, read back the manifest/digest, then answer exactly `RECOVERY_READY run=<run-id> state=<reconstructed-phase> canary_set=<sha256>` without advancing work.
...
- The approval retry limit is exactly `1`. For each required surface, prove a unique path absent; create first bytes and read back exact path/bytes/SHA-256; change the same artifact through the intended role mutation route and verify distinct second bytes/hash; remove only that artifact and a canary-created empty parent; then prove path absence and exact surrounding-state restoration. Before any index-writing operation, the Worker captures the resolved index's complete raw-byte preimage and requires its SHA-256 to equal the bound initial value. It stages only a separate initially absent unignored path, verifies exact index path/mode/blob/content, unstages only it, removes it, restores the exact preimage through the bounded Git-index route without retaining a backup artifact, and proves final raw index length/SHA-256 after all remaining identity reads plus complete initial/final HEAD, status, index tree, and pre-existing identities equal. Semantic index equality never substitutes for raw-byte equality. No commit, GitHub mutation, source edit, existing path, user work, or route/helper substitution is allowed.
- Before each canary, validate a literal `LEGACY_BOOTSTRAP` context against live task, Git, and filesystem evidence. Bind `source_contract` to the SHA-256 identity of this exact owner-delivered literal bootstrap protocol; set both `active_contract_id` and `contract_bundle` to `none-before-legacy-contract` only after proving that no activation ID, legacy record, contract bundle, prepared record, or committed record exists. Bind every retained active leaf and branch, or `none` only when that resource is genuinely absent. The Orchestrator binds the authoritative checkout as its worktree; an applicable retained Worker binds its exact linked worktree. Also bind the exact run, role/task metadata, execution profile, runtime, initial Git identities, target paths, and approval limit. Any missing, invented, stale, or contradictory value returns `CONTEXT_MISMATCH` before mutation.
- Before dispatching each literal canary turn, independently reread the stable role task/profile. On that exact turn and before action, require the role to invoke immutable metadata, bind the new turn only as `metadata_turn`, and stop on mismatch. After return, independently read back that exact populated task/turn/profile and reject the result before aggregate acceptance or transition on any mismatch.
- For every Worker canary, verify `worker_runtime` from the actual task. When it is `NATIVE_WINDOWS`, create, change, and delete every canary file only through the dedicated `apply_patch` tool in that Worker's normal writable-worktree sandbox; never use PowerShell, a shell/pipeline, a here-string/here-document, a `.bat`/`.cmd` wrapper, or `require_escalated` for those file mutations. If direct file deletion leaves only the exact canary-created parent, the Worker may return `WINDOWS_CANARY_EMPTY_PARENT_READY` only after proving the parent was initially absent, is the expected nonce-bound ordinary non-reparse descendant inside but unequal to its linked worktree, contains zero entries including hidden/system entries, and the raw index and surrounding identities are restored. The external creator independently verifies that mutation-turn evidence, removes only that exact parent nonrecursively and without force, reads back absence, rereads the same stable Worker task/profile, and sends one fresh metadata-gated read-only finalization turn. Only that turn may return the canonical result after repeated parent/artifact absence, raw-index/worktree restoration, and no-transition verification; its evidence digest binds both turns and the external operation. The one configured approval retry remains a total bound for the canary. Ambiguity or incomplete finalization is `FILESYSTEM_CAPABILITY_UNAVAILABLE`. This native-Windows exception never permits deletion of a file, source, ancestor, sibling, worktree root, task anchor, `.roundlet` state, repository root, or user path, and it never permits recursion, force, or target broadening. If the required file route is unavailable, make no shell/elevated file retry, preserve truthful cleanup state, and return `FILESYSTEM_CAPABILITY_UNAVAILABLE`. This conditional adds no patch-routing or empty-parent-finalization rule for `WSL` or `NON_WINDOWS`; exact-path Git stage/unstage remains a separate normal-sandbox index operation.
...
- The approval retry limit is exactly `1`. For each required file surface, prove a unique path absent; create first bytes and verify exact path/bytes/SHA-256; change the same artifact through the intended role route and verify distinct second bytes/hash; remove only it and a canary-created empty parent; then prove absence and exact surrounding-state restoration. Before any index-writing operation, the Worker captures the resolved index's complete raw-byte preimage and requires its SHA-256 to equal the bound initial value. It stages only a separate initially absent unignored path, verifies exact index path/mode/blob/content, unstages only it, removes it, restores the exact preimage through the bounded Git-index route without retaining a backup artifact, and proves final raw index length/SHA-256 after all remaining identity reads plus complete initial/final HEAD, status, index tree, and pre-existing identities equal. Semantic index equality never substitutes for raw-byte equality. No commit, GitHub mutation, source edit, existing path, user work, or helper/route substitution is allowed.
- For the new short-lived Worker, verify `worker_runtime` from the actual task. When it is `NATIVE_WINDOWS`, first bind its exact host-owned task-anchor CWD and create the removable linked worktree as a distinct writable descendant; never make the task CWD equal to or inside that worktree. Then create, change, and delete every canary file only through the dedicated `apply_patch` tool in that Worker's normal writable-worktree sandbox; never use PowerShell, a shell/pipeline, a here-string/here-document, a `.bat`/`.cmd` wrapper, or `require_escalated` for those file mutations. If direct file deletion leaves only the exact canary-created parent, the Worker may return `WINDOWS_CANARY_EMPTY_PARENT_READY` only after proving the parent was initially absent, is the expected nonce-bound ordinary non-reparse descendant inside but unequal to its linked worktree, contains zero entries including hidden/system entries, and the raw index and surrounding identities are restored. The external creator independently verifies that mutation-turn evidence, removes only that exact parent nonrecursively and without force, reads back absence, rereads the same stable Worker task/profile, and sends one fresh metadata-gated read-only finalization turn. Only that turn may return the canonical result after repeated parent/artifact absence, raw-index/worktree restoration, and no-transition verification; its evidence digest binds both turns and the external operation. The one configured approval retry remains a total bound for the canary. Ambiguity or incomplete finalization is `FILESYSTEM_CAPABILITY_UNAVAILABLE`. This exception never permits deletion of a file, source, ancestor, sibling, worktree root, task anchor, `.roundlet` state, repository root, or user path, and it never permits recursion, force, or target broadening. If that topology or file route is unavailable, make no shell/elevated file retry, preserve truthful cleanup state, and return `FILESYSTEM_CAPABILITY_UNAVAILABLE`. This entire topology, patch-routing, and empty-parent-finalization conditional adds no rule for `WSL` or `NON_WINDOWS`; their existing platform-appropriate flow remains unchanged, and exact-path Git stage/unstage remains a separate normal-sandbox index operation.
- Before dispatching each literal canary turn, independently reread the stable role task/profile. On that exact turn and before action, require the role to invoke immutable metadata, bind the new turn only as `metadata_turn`, and stop on mismatch. After return, independently read back that exact populated task/turn/profile and reject the result before aggregate acceptance or transition on any mismatch.
- Store each result as exact UTF-8 without BOM or trailing newline with these lines in this order: `FILESYSTEM_CANARY_RESULT`; `phase: BETWEEN_ISSUES_ADOPTION`; `role: <ORCHESTRATOR|WORKER>`; `run_id: <exact-run-id>`; `role_task: <exact-task-id>`; `metadata_turn: <metadata-read-exact-populated-turn-id>`; `execution_profile: model=<task-metadata-model>;reasoning_effort=<task-metadata-effort>`; `host_route_fingerprint: <task-host-checkout-worktree-permission-route-tool-class-digest>`; `target_paths: <exact-local-canary-paths>`; `advisory_surface: <PASS|NOT_APPLICABLE|FAIL> <evidence-digest-or-none>`; `worktree_surface: <PASS|NOT_APPLICABLE|FAIL> <initial-final-identity-digest-or-none>`; `index_surface: <PASS|NOT_APPLICABLE|FAIL> <initial-final-index-and-entry-digest-or-none>`; `approval_retry_count: <0|1>`; `approval_outcome: <NOT_REQUIRED|APPROVED|ESCALATION_DENIED|ESCALATION_UNAVAILABLE>`; `execution_outcome: <SUCCEEDED|NOT_LAUNCHED|DIRECT_EXECUTION_FAILED|ESCALATED_EXECUTION_FAILED>`; `capability_outcome: <PASS|FILESYSTEM_CAPABILITY_UNAVAILABLE>`; `cleanup: <VERIFIED|FAILED> <evidence-digest>`; `repository_transition: none`.
...
a short-lived candidate-configured Worker plus temporary linked worktree to pass the unique
worktree file and exact Git-index stage/read-back/unstage canaries with exact artifact/index cleanup proof. Complete that new Worker's metadata-only handshake and creator-side registry read-back before its populated canary turn. On native Windows only, use the literal protocol's separated host-owned task anchor and descendant linked-worktree topology; other runtimes keep their existing topology. Aggregate both role results as the exact BETWEEN_ISSUES_ADOPTION evidence set and read back its manifest/digest; only then archive the clean short-lived Worker and apply bounded external task/worktree cleanup read-back, including exact-current-directory holders, Git registration, unique work, and physical-path absence. The completed-transition set remains immutable evidence for this preparation but cannot authorize a later transition. Classify typed outcomes and stop before contract work on any failure. Verify task metadata shows this
...
- For every Worker canary, verify `worker_runtime` from the actual task. When it is `NATIVE_WINDOWS`, create, change, and delete every canary file only through the dedicated `apply_patch` tool in that Worker's normal writable-worktree sandbox; never use PowerShell, a shell/pipeline, a here-string/here-document, a `.bat`/`.cmd` wrapper, or `require_escalated` for those file mutations. If direct file deletion leaves only the exact canary-created parent, the Worker may return `WINDOWS_CANARY_EMPTY_PARENT_READY` only after proving the parent was initially absent, is the expected nonce-bound ordinary non-reparse descendant inside but unequal to its linked worktree, contains zero entries including hidden/system entries, and the raw index and surrounding identities are restored. The external creator independently verifies that mutation-turn evidence, removes only that exact parent nonrecursively and without force, reads back absence, rereads the same stable Worker task/profile, and sends one fresh metadata-gated read-only finalization turn. Only that turn may return the canonical result after repeated parent/artifact absence, raw-index/worktree restoration, and no-transition verification; its evidence digest binds both turns and the external operation. The one configured approval retry remains a total bound for the canary. Ambiguity or incomplete finalization is `FILESYSTEM_CAPABILITY_UNAVAILABLE`. This native-Windows exception never permits deletion of a file, source, ancestor, sibling, worktree root, task anchor, `.roundlet` state, repository root, or user path, and it never permits recursion, force, or target broadening. If the required file route is unavailable, make no shell/elevated file retry, preserve truthful cleanup state, and return `FILESYSTEM_CAPABILITY_UNAVAILABLE`. This conditional adds no patch-routing or empty-parent-finalization rule for `WSL` or `NON_WINDOWS`; exact-path Git stage/unstage remains a separate normal-sandbox index operation.
- Before dispatching each literal canary turn, independently reread the stable role task/profile. On that exact turn and before action, require the role to invoke immutable metadata, bind the new turn only as `metadata_turn`, and stop on mismatch. After return, independently read back that exact populated task/turn/profile and reject the result before aggregate acceptance or transition on any mismatch.
- Store each result as exact UTF-8 without BOM or trailing newline with these lines in this order: `FILESYSTEM_CANARY_RESULT`; `phase: ACTIVE_IN_PLACE_MIGRATION`; `role: <ORCHESTRATOR|WORKER>`; `run_id: <exact-run-id>`; `role_task: <exact-task-id>`; `metadata_turn: <metadata-read-exact-populated-turn-id>`; `execution_profile: model=<task-metadata-model>;reasoning_effort=<task-metadata-effort>`; `host_route_fingerprint: <task-host-checkout-worktree-permission-route-tool-class-digest>`; `target_paths: <exact-local-canary-paths>`; `advisory_surface: <PASS|NOT_APPLICABLE|FAIL> <evidence-digest-or-none>`; `worktree_surface: <PASS|NOT_APPLICABLE|FAIL> <initial-final-identity-digest-or-none>`; `index_surface: <PASS|NOT_APPLICABLE|FAIL> <initial-final-index-and-entry-digest-or-none>`; `approval_retry_count: <0|1>`; `approval_outcome: <NOT_REQUIRED|APPROVED|ESCALATION_DENIED|ESCALATION_UNAVAILABLE>`; `execution_outcome: <SUCCEEDED|NOT_LAUNCHED|DIRECT_EXECUTION_FAILED|ESCALATED_EXECUTION_FAILED>`; `capability_outcome: <PASS|FILESYSTEM_CAPABILITY_UNAVAILABLE>`; `cleanup: <VERIFIED|FAILED> <evidence-digest>`; `repository_transition: none`.
- Retry only an initial restriction once through the narrowest host-supported approval for the same target/operation. The only valid `(approval_retry_count, approval_outcome, execution_outcome)` tuples are `(0, NOT_REQUIRED, SUCCEEDED)`, `(0, NOT_REQUIRED, DIRECT_EXECUTION_FAILED)`, `(1, APPROVED, SUCCEEDED)`, `(1, APPROVED, ESCALATED_EXECUTION_FAILED)`, `(1, ESCALATION_DENIED, NOT_LAUNCHED)`, and `(0, ESCALATION_UNAVAILABLE, NOT_LAUNCHED)`. PASS requires execution `SUCCEEDED`, every required surface PASS, only inapplicable surfaces NOT_APPLICABLE, capability PASS, and cleanup VERIFIED. Every other valid tuple and every failure requires `FILESYSTEM_CAPABILITY_UNAVAILABLE`; cleanup remains independently truthful and external cleanup never changes it.
