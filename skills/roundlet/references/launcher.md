...
Read the complete Roundlet SKILL.md and every reference it requires before acting. Require the preceding immutable task-metadata handshake from `thread-prompts.md` to have exhausted deferred discovery and returned this exact Launcher task/model/effort. Require the external creator's independent immutable task/turn-record read-back to equal those values; missing eager exposure, self-reported profile text, or the child response alone is insufficient. Reread the immutable metadata route on this exact populated turn, require it to equal the owner-bound expected values above, and bind the verified profile plus actual task ID into the Launcher canary result. Do not implement an issue in this Launcher task.
