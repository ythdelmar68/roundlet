...
- For every Worker canary, verify `worker_runtime` from the actual task. When it is `NATIVE_WINDOWS`, create, change, and delete every canary artifact only through the dedicated `apply_patch` tool in that Worker's normal writable-worktree sandbox; never use PowerShell, a shell/pipeline, a here-string/here-document, a `.bat`/`.cmd` wrapper, or `require_escalated` for those mutations. If that route is unavailable, make no shell/elevated patch retry, preserve truthful cleanup state, and return `FILESYSTEM_CAPABILITY_UNAVAILABLE`. This conditional adds no patch-routing rule for `WSL` or `NON_WINDOWS`; exact-path Git stage/unstage remains a separate normal-sandbox index operation.
