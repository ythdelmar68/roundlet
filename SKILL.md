...
Use `discover_membership`, `parse_required_order`, and `parse_dependency_edges`, then call the state-mutating `select_next_task` exactly once with a complete ordered per-umbrella refresh manifest. Pass exact connector-returned umbrella, membership, issue, and completion receipts—not caller-selected source labels or booleans. Every receipt must bind repository name/ID, umbrella and issue IDs, one refresh timestamp, and canonical source data. The validator recomputes umbrella and issue revisions plus each membership-evidence digest, and rejects unknown source types, stale or synthetic digests, non-boolean snapshot fields, missing umbrella coverage, partial/empty manifest evidence, umbrella-as-task, and hand-built selection receipts. A discovered issue counts complete only when the same refresh contains a live `closed/completed` issue receipt and a same-repository `closed+merged` PR receipt. Apply hard dependencies first, then required within-umbrella order. Across umbrella heads, use cross-dependency impact, unlocked work, overlap risk, owner priority, activation umbrella order, and issue number.
