...
Use `discover_membership`, `parse_required_order`, and `parse_dependency_edges`, then call the state-mutating `select_next_task` exactly once with a complete ordered per-umbrella refresh manifest. Require repository/base identity, every authorized umbrella revision, membership-evidence digest, complete discovered-issue list, and every sub-issue revision. The API binds canonical snapshot evidence and a receipt digest into state. Reject missing umbrella coverage, partial/empty manifest evidence, umbrella-as-task, hand-built receipts, and completion without the final complete receipt. Apply hard dependencies first, then required within-umbrella order. Across umbrella heads, use cross-dependency impact, unlocked work, overlap risk, owner priority, activation umbrella order, and issue number.
