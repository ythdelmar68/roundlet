...
7. Verify `.codex-log/roundlet/` is ignored and initialize bounded state atomically.
...
Normalize the activation with `normalize_activation_request`, resolve identity with `resolve_repository_identity`, and initialize `state.json` with `new_state` plus `StateStore.initialize`. Use an owner-visible stable activation ID and the exact installed digest returned by `skill_content_digest`.
...
Build membership only from formal same-repository sub-issue links, explicit umbrella body lists/matrices/orders, or owner-authored umbrella comments. Do not let a third-party comment or external link expand scope.

Use `discover_membership`, `parse_required_order`, `parse_dependency_edges`, and `select_next_task` to create a bounded selection receipt. Apply hard dependencies first, then required within-umbrella order. Across umbrella heads, use cross-dependency impact, unlocked work, overlap risk, owner priority, activation umbrella order, and issue number.
...
Immediately read the newly created task back from the task service and pass its exact task ID, service creation timestamp, activation, issue, and next review generation to `begin_supervisor` as the immutable creation receipt. Never synthesize or copy that receipt, accept an older/non-monotonic creation time, fork the Worker, or reuse a previous Supervisor ID.
