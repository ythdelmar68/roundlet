...
7. Call state-mutating `select_next_task` once. It recomputes umbrella and issue revisions and each per-umbrella membership-evidence digest from canonical receipts, strictly validates all snapshot field types, and binds the manifest, snapshots, decision, activation/base, and receipt digest into state. A discovered issue is complete only when the same refresh includes live `closed/completed` issue evidence and a same-repository `closed+merged` PR receipt. Do not assign `state["selection"]` manually or transition to `scope-complete` without its final complete receipt.
