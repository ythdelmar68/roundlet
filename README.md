...
- **Immutable task metadata** uses a two-stage handshake. Missing eager tool exposure is inconclusive: the role exhausts deferred discovery, reads immutable task/model/effort metadata in a metadata-only turn, the external creator independently verifies an immutable task/turn record, and the role rereads metadata on the exact populated turn before acting.
