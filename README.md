...
- **Native Windows Worker patch routing** keeps canary-file creation/change/deletion and source edits on the dedicated `apply_patch` tool inside the assigned writable worktree's normal sandbox. It forbids PowerShell/pipeline/here-string/batch-wrapper and elevated-shell patch fallbacks because those host-specific helper routes can fail independently of worktree capability. When direct file cleanup leaves only an initially absent, nonce-bound, ordinary, non-reparse, proven-empty canary parent, a Windows-only two-turn exception lets the external creator remove precisely that directory nonrecursively and without force; the same Worker then performs a fresh metadata-gated read-only finalization before cleanup can be verified. The exception cannot delete files, source, ancestors, worktrees, anchors, or run state. This guard applies only to native Windows, not WSL or other platforms, and it does not prohibit narrowly approved elevation for genuinely host-bound GitHub, network, or out-of-root operations.
