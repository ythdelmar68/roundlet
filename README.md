...
Open a new Codex task at the exact owner-selected Launcher profile and first send only the immutable task-metadata handshake from [`thread-prompts.md`](skills/roundlet/references/thread-prompts.md#immutable-task-metadata-handshake). After the task exhausts deferred discovery, returns `TASK_METADATA_READY`, and the creator independently verifies an immutable task/turn record with the same task ID, model, and reasoning effort, send this wrapper in that same task. It tells Codex to load the complete canonical activation prompt from the installed skill, so the long contract has one source of truth:
...
- **Immutable task metadata** uses a two-stage handshake. Missing eager tool exposure is inconclusive: the role exhausts deferred discovery, accepts `role_task` only from equal thread/session fields, records `turn_id` separately, the external creator independently verifies the immutable task/turn/profile record, and the role rereads metadata on the exact populated turn before acting. A turn ID can never substitute for a task ID.
- **Raw index restoration** treats `initial_index_sha256` as an exact byte preimage. A Worker captures the resolved linked-worktree index before staging, restores those same bytes after exact-path unstage, and verifies the final raw hash after all remaining identity reads. Matching HEAD, status, index tree, and entries is necessary but cannot substitute for byte equality.
- **Task/worktree cleanup** is separate from artifact/index cleanup and from an archive acknowledgement. Bounded external read-back must prove the task non-active, exact Git registration absent, no unique work, no live process retaining the exact worktree as its current directory, and the physical path absent. Roundlet never kills Codex/Node or force-removes a path to manufacture `cleanup=VERIFIED`.
...
- On native Windows, failure of a shell-wrapped or elevated patch helper is not proof that the assigned worktree is unwritable or that approval was denied. Workers must use the dedicated normal-sandbox patch route for canary artifacts and source edits, and fail closed as `FILESYSTEM_CAPABILITY_UNAVAILABLE` instead of retrying either mutation through elevation. WSL and non-Windows Workers retain their existing platform-appropriate edit routes.
...
- Cleanup is part of the active issue. No next issue is selected until the checkout is clean, issue-specific tasks are non-active, no exact-current-directory holder or worktree registration/path remains, resources are removed as authorized, and `HEAD == main == origin/main`.
